...
while rematch==1
    ...
    if turns==1
        while (count~=Length-1 && turns~=0) 
            ...
            %function: rightMatch checks if letter typed is correct, correct guessed letters displayed and 
            %displays other results so far
    ...
    if count==Length-1
        while turns~=0 && count~=0 
            ...
            %if only one turn left and its a wrong guess, player loses and asked if they want to play again, 
            %accepts yes or no only 
            elseif turns==1
